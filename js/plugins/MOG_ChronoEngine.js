...
//==============================
// * remove Tools On Map /***修改 */
//==============================
Game_Map.prototype.removeToolsOnMap = function(tool_id) {
    for (var i = $gameSystem._toolsOnMap.length; i >= 0; i--) {
        var toolMapID = $gameSystem._toolsOnMap[i];
...
Game_CharacterBase.prototype.setDeadEnemy = function(char, battler) {
    ...
    char._opacity = 255
    var item = this.makeTreasureItem(char,battler)
    this.setTreasure(char,item)
...
Game_CharacterBase.prototype.setTreasure = function(char,item,x,y) {
    
    if(char&&item){ 
        var char = JsonEx.parse(JsonEx.stringify(char)) 
        var battler = char.battler()
    
        var x = (( x=== undefined)? char._realX : x ||0)  
        var y = (( y=== undefined)? char._realY : y ||0)

        char.setPosition(x,y) 

        this.makeTreasure(char, item);
        ...
        if (!char._user.treasure[0]) {
            ...
            if ($gameSystem.isChronoMode()) { char.jump(0, 0) };
            char._isaddTreasure = true
            $gameMap._treasureAdd = $gameMap._treasureAdd||[]
            $gameMap._treasureAdd.push(char)
        };
        char._chrono.deadChecked = true;
        char.battler().performCollapse();
        char._user.rotation = [0, 0]; 
...
//==============================
Game_CharacterBase.prototype.makeTreasureItem = function(char, battler) {
    ...
    return item //||$dataItems[4]
};



Game_CharacterBase.prototype.makeTreasure = function(char, item) { 
    var item =  item
...
Game_Player.prototype.commandAttackUsable = function() {
    ...
    if (Input.isRepeated(Moghunter.ras_buttonWeapon)) { return true };
    return false;
};
    ...
    if (!this._erased && this.page()) {
        this.list().forEach(function(l) {
            if (l.code === 108) {
                ...
                if (comment[0].toLowerCase() == "usersetup") { 
                    this.userSetup();
                    this.toolSetup();
                };  
                if (comment[0].toLowerCase() == "dead selfswitch id") {
                    if (this._user.battler) {
                        this._user.battler._ras.deadSelfSwitchID = String(comment[1]);
                    }
                };
    ...
    if (scopeFriend) {
        if (scope === 11) {
            for (var i = 0; i < allies.length; i++) {
                ...
                if (abattler === battler) {
                    battler._chrono.targets.push(allies[i])
                };
            };
        } else if (scope === 10) {
...
ToolEvent.prototype.deadEffectActor = function(char, battler) {
    ...
    if (!$gameSystem.isChronoMode()) {
        if ($gameParty.leader()._actorId === battler._actorId) {
            ...
            if ($gameParty.leader().isDead()) { $gameTemp.reserveCommonEvent(44) };
...
Game_Character.prototype.moveDiagonally = function(horz, vert) {
    ...
    if (!this.isMovementSucceeded() && !this._tool.enabled) {
        this.moveDiagonallyAfter(horz, vert);
...
Game_CharacterBase.prototype.getTreasure = function(event) {
    ...
    if(event._isaddTreasure){ 
        event._isaddTreasureDel = true 
        $gameMap._treasureAdd = $gameActors._treasureAdd||[]
        $gameMap._treasureAdd.push(event)
    } 
};
...
Spriteset_Map.prototype.updateChronoSprites = function() {
    ...
    if($gameMap._treasureAdd){
        this.createTreasure()
    }
};

Spriteset_Map.prototype.createTreasure = function() { 
    var l = $gameMap._treasureAdd
    if(l){
        for(var i = 0 ; i<l.length;i++){
            var e = l[i]
            if(e._isaddTreasureDel){
                var  s = e._treasureAddSprite
                if(s){
                    this._tilemap.removeChild(s)
                } 
                s._character = null
                e._treasureAddSprite = null
            }else{
                var s = new Sprite_Character(e)
                e._treasureAddSprite = s 
                this._tilemap.addChild(s)
            }
        }
    } 
    console.log(l)
    $gameMap._treasureAdd = null
};

 ...
 * @desc 当在悬停事件中带有移动播放器命令时，自动显示此自定义光标映像。
 *
 ...
 * @desc 在具有更改项命令的事件悬停时自动显示此自定义光标映像。.
 *
 * @param Change Weapons Cursor
 * @desc 当悬停在具有更改武器命令的事件时，自动显示此自定义光标映像。.
 *
 * @param Change Armors Cursor
 * @desc Automatically show this custom cursor image when hovering over events with Change 盔甲 commands in them.
 *
 * @param Battle Processing Cursor
 * @desc 光标悬停在这个自动显示在他们的战斗命令和事件处理。
 *
 * @param ---Auto Change Icons---
 * @desc 选项的自动显示一个图标时，在给定的事件在事件在他们的命令.
 * @default
 *
 * @param Show Text Icon
 * @desc 当悬停在具有显示文本命令的事件时，自动显示此图标.
 *
 * @param Transfer Icon
 * @desc 当悬停在带有传输命令的事件时，自动显示此图标。 .
 *
 ...
 * @desc 当悬停时高亮显示时，播放光标SE之间的音频冷却（帧）设置为true。默认值为4。
 * @default 4
 *
 * @param ---Customizeable Notetags---
 * @desc 这些是通过鼠标交互而不是玩家角色激活事件的选项。
 * @default
 *
 * @param No Auto Cursor Notetag
 * @desc 用于禁用自动光标切换此事件或活动页面。
 * @default no_auto_cursor!
 *
 * @param No Auto Icon Notetag
 * @desc 用于禁用自动图标切换此事件或活动页面。
 ...
 *
 * @param Icon Tag 1
 * @desc 建立一个快捷方式图标标签使用鼠标悬停图标notetag。有关更多信息，请参见插件帮助.
 * @default quest: 191
 *
 * @param Icon Tag 2

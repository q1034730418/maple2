 ...
 * data  同上眠的第二个参数
 * 
 * can   [index,value,index,value]  
 * 对于index的图标是否为value ,value为true/false
 * 如果不符合则不运行
 * 
 * next  0 如果满足can 则不进行下一个,  1 满足也进行下应该 2 不满足则不进行下一个
 * 
 * 第一个为 抬起瞬间的处理 
 * 第二个数组为 按下中的处理
 * 举例  
 *  
 * 
Galv.SBTNS.addButton(1, 'map', 'BtnOk', 570, 500, ['more',
    [
        [
            'script', 'console.log("2未按下中1单击")', [2, false], 0,
            'script', 'console.log("2按下中1单击")', [2, true], 0
        ], 
    ]

]);
Galv.SBTNS.addButton(2, 'map', 'BtnRun', 690, 500, ['more',
    [
        [
            'script', 'console.log("1未按下中2单击")', [1, false], 0,
            'script', 'console.log("1按下中2单击")', [1, false], 0
        ],  [
            'script', 'console.log("2按下1未按下")', [1, false], 0,
            'script', 'console.log("2按下1按下了")', [1, true], 0
        ], 
    ]
]);
...
(function () {
    ...
    Galv.SBTNS._currentState = {};


    Galv.SBTNS.onClick = function (button, value) {
        var z = Galv.SBTNS._currentState[button]
        if (value !== undefined) {
            Galv.SBTNS._currentState[button] = value
        }
        return z
    }

    Galv.SBTNS.isClick = function (button, value) {
        return Galv.SBTNS._currentState[button] == value

    }

    Galv.SBTNS.clear = function () {
        Galv.SBTNS._currentState = {}
    }
        ...
        if (obj.action) {
            ...
            switch (type) {
                ...
                case 'event': // for common event
                    ...
                    break;
                case 'more': // for common event
                    if (Array.isArray(data)) {

                        if (data[0]) {
                            this._GButtons[index].setClickHandler(Galv.SBTNS.runMore.bind(this, data[0]));
                        }
                        if (data[1]) {
                            this._GButtons[index].setPressHandler(Galv.SBTNS.runMore.bind(this, data[1]));
                        }
                    }
                    break;
            }
    ...
    Galv.SBTNS.runMore = function (action) {
        if (Array.isArray(action)) {
            for (var i = 0; i < action.length; i += 4) {
                var type = action[i]
                var data = action[i + 1]
                var can = action[i + 2]
                var next = action[i + 3]
                if (Galv.SBTNS.runCan(can)) {
                    Galv.SBTNS.runMoreDo(type, data)
                    if (!next) {
                        return
                    }
                } else if (next == 2) {
    ...
    Galv.SBTNS.runCan = function (can) {
        if (Array.isArray(can)) {
            for (var i = 0; i < can.length; i += 2) {
                var index = can[i]
                var value = can[i + 1]
                if (!Galv.SBTNS.isClick(index, value)) {
                    return false
                }
            }
        }
    ...
    Galv.SBTNS.runMoreDo = function (type, data) {

        switch (type) {
            case 'press': // for button press emulation
                Galv.SBTNS.btnPress.bind(this, data)
                break;
            case 'trigger': // for button trigger emulation
                Galv.SBTNS.btnTrigger(data)
                break;
            case 'release':
                Galv.SBTNS.btnRelease(data)
                break;
            case 'eval': // for script calls
            case 'script': // for script calls
                Galv.SBTNS.gButtonScript(data);
                break;
            case 'event': // for common event
                Galv.SBTNS.runGCommentEvent(data);
                break;
...
Sprite_GButton.prototype.processTouch = function () {
    if (this.isActive() && this.opacity >= 255) {
        ...
        if (TouchInput.isTriggered() && this.isButtonTouched()) {
            ...
            Galv.SBTNS.onClick(this._btn.id, true)
        }
        //如果触摸瞬间
        if (this._touching) {
            //如果是重复 , 离开
            if (TouchInput.isReleased() || !this.isButtonTouched()) {
                //如果是触摸在之中
                Galv.SBTNS.onClick(this._btn.id, false)

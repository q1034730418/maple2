...
(function() {
    ...
    Window_MiniInfo.prototype.processDrawIcon = function(iconIndex, textState) {
        ...
        textState.x += this.standardFontSize() + 32;
    };
    ...
    Window_MiniInfo.prototype.drawIcon = function(iconIndex, x, y) {/*
        ...
        var n = Math.floor((this.contents.fontSize / 15) * Window_Base._iconWidth);
        this.contents.blt(bitmap, sx, sy, pw, ph, 140, y, n, n);*/
    };
    ...
    Window_MiniInfo.prototype.makeContents = function() {
        ...
        if (item.effects) {
            for (var i = 0, max = item.effects.length; i < max; i++) {
                ...
                switch (e.code) {
                    case 11:
                        if (e.value1 > 0 && effectNames[0]) text = s + effectNames[0] + ':' + g ;
                        if (e.value1 < 0 && effectNames[1]) text = s + effectNames[1] + ':' + r ;
                        if (e.value2 > 0 && effectNames[0]) text = s + effectNames[0] + ':' + g + e.value2;
                        if (e.value2 < 0 && effectNames[1]) text = s + effectNames[1] + ':' + r ;
                        break;
                    case 12:
                        if (e.value1 > 0 && effectNames[2]) text = s + effectNames[2] + ':' + g ;
                        if (e.value1 < 0 && effectNames[3]) text = s + effectNames[3] + ':' + r ;
                ...
                switch (trait.code) {
                    ...
                    case 112:
                        if (vocab[0][1] && value !== 0) {
                            var param = TextManager.param(dataId);
                            du = value < 0 ? g : r;
                            text = c + param + s + vocab[0][1] + du + sym ;
                        }
                        break;
                    case 113:
                        if (vocab[0][0] && value !== 0) {
                            var state = $dataStates[dataId].name;
                            du = value < 0 ? g : r;
                            text = c + state + s + vocab[0][0] + du + sym ;
                        }
                        break;
                    case 121:
                        if (value !== 0) {
    ...
    Window_MapSelectEquip.prototype.callUpdateHelp = function() {
        if (this.active && this._miniInfoWindow) {
            this.updateHelp();
        }
    };
    ...
    Scene_Base.prototype.createMiniWindow = function() {
        ...
        if (this._MapSelectEquipWindow) {
            this._MapSelectEquipWindow._miniInfoWindow = this._miniWindow;
        }
        this.addChild(this._miniWindow);
